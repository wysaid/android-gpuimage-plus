...
namespace CGE
{
	...
	void CGETiltshiftVectorFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, const GLfloat* posVertices)
	{
		handler->setAsTarget();
		m_program.bind();
	...
	void CGETiltshiftEllipseFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, const GLfloat* posVertices)
	{
		handler->setAsTarget();
		m_program.bind();
		const CGESizei& sz = handler->getOutputFBOSize();
