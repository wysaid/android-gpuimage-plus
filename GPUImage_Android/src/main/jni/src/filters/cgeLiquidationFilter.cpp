...
namespace CGE
{
	...
	void CGELiquidationFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, const GLfloat* posVertices)
	{
	...
	void CGELiquidationFilter::forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity)
	{
		...
		float loopStartY = CGE_MAX(CGE_MIN(start.y(), end.y()) - radius, -radius);
		float loopEndY = CGE_MIN(CGE_MAX(start.y(), end.y()) + radius, h + radius);
		float loopStartX = CGE_MAX(CGE_MIN(start.x(), end.x()) - radius, -radius);
		float loopEndX = CGE_MIN(CGE_MAX(start.x(), end.x()) + radius, w + radius);
