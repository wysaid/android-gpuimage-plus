...
namespace CGE
{
	...
	void CGESharpenBlurFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, const GLfloat* posVertices)
	{
	...
	void CGEBlurFastFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint dstTexture, const GLfloat* posVertices)
	{
		const CGESizei& sz = handler->getOutputFBOSize();
	...
	void CGESharpenBlurFastFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, const GLfloat* posVertices)
	{
		handler->setAsTarget();
		m_program.bind();
		const CGESizei& sz = handler->getOutputFBOSize();
		if(m_texture == 0)
		{
			m_texture = cgeGenTextureWithBuffer(NULL, sz.width, sz.height, GL_RGBA, GL_UNSIGNED_BYTE);
			m_blurProc.render2Texture(handler, m_texture, posVertices);
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, handler->getTargetTextureID(), 0);
		}

		CGEImageFilterInterface::render2Texture(handler, srcTexture, posVertices);
	}
