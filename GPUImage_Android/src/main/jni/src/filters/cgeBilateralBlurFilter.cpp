...
namespace CGE
{
	...
	void CGEBilateralWrapperFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, const GLfloat* posVertices)
	{
		assert(m_proc != NULL); // Filter 尚未初始化成功
		...
		{
			m_proc->render2Texture(handler, srcTexture, posVertices);
