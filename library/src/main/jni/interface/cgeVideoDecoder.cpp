...
struct CGEVideoDecodeContext
{
    CGEVideoDecodeContext() :
        pFormatCtx(nullptr), pVideoCodecCtx(nullptr), pAudioCodecCtx(nullptr), pVideoCodec(nullptr), pAudioCodec(nullptr), pVideoFrame(nullptr), pVideoFrameRGB(nullptr), pAudioFrame(nullptr), pVideoStream(nullptr), pAudioStream(nullptr), videoStreamIndex(-1), audioStreamIndex(-1), pSwrCtx(nullptr) {}
    ~CGEVideoDecodeContext()
    {
        cleanup();
    }
...
CGEVideoDecodeHandler::CGEVideoDecodeHandler() :
    m_bufferPtr(nullptr), m_width(0), m_height(0), m_samplingStyle(ssFastBilinear), m_currentTimestamp(0.0)
{
