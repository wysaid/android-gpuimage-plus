...
CGEThreadPreemptive::CGEThreadPreemptive() :
    m_thread(nullptr), m_taskRestart(false), m_threadOver(false), m_runningStatus(false)
{
}
...
CGEThreadPool::CGEThreadPool(std::list<std::unique_ptr<Worker>>::size_type maxWorkerNum) :
    m_threadOver(false), m_maxWorkerSize(maxWorkerNum), m_threadJoining(false)
{
...
CGEThreadPool::Worker::Worker(CGEThreadPool& pool) :
    m_thread(nullptr), m_pool(pool), m_runningStatus(false), m_shouldLeave(false)
{
}

CGEThreadPool::Worker::Worker(Worker& worker) :
    m_pool(worker.m_pool)
{
}
