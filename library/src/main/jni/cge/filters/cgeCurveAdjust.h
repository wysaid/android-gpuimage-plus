...
public:
    ...
    inline const float* getCurveRGB(size_t* len = nullptr) const
    {
        if (len != nullptr) *len = m_curve.size();
        return &m_curve[0][0];
    };
...
{
public:
    CGEMoreCurveTexFilter() :
        m_curveTexture(0) {}
    virtual ~CGEMoreCurveTexFilter()
    {
        glDeleteTextures(1, &m_curveTexture);
        m_curveTexture = 0;
    }
